  ...
  return (
    <section id="home" className="relative overflow-hidden" aria-label="Hero">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute -top-32 left-1/2 h-[720px] w-[720px] -translate-x-1/2 rounded-full bg-white/6 blur-3xl" />
        <div className="absolute top-40 left-1/3 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-gold/10 blur-3xl" />
      </div>
      ...
      <Container>
        <div className="pt-14 md:pt-20 lg:pt-24 pb-10 md:pb-14">
          <MotionDiv variants={stagger} initial="hidden" animate="show" className="relative z-10 max-w-[1040px]">
            ...
            <MotionP variants={fadeUp} className="mt-6 text-[12px] uppercase tracking-luxe text-ash">
              {t.suborg}
            </MotionP>

            <MotionH1
              variants={fadeUp}
              className="mt-5 font-display text-[38px] leading-[1.02] md:text-[76px] lg:text-[96px] tracking-[-0.03em]"
            >
              {t.statementTop}
            ...
            <MotionDiv variants={fadeUp} className="mt-10 flex flex-col sm:flex-row gap-3 sm:items-center">
              <button
                type="button"
                onClick={() => openContact()}
                className="inline-flex w-full sm:w-auto items-center justify-center rounded-full border border-hairline px-6 py-3 text-[12px] uppercase tracking-luxe text-paper hover:bg-paper hover:text-ink transition-colors"
              >
                {t.ctaPrimary}
              </button>
              <a
